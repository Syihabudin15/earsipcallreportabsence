...
export default function DataInsurance() {
  ...
  const getData = async () => {
    ...
    await api
      .request({
        url: `${import.meta.env.VITE_API_URL}/insurance?${params}`,
        method: "GET",
      })
      .then((res) =>
        setPageprops((prev) => ({
  ...
  return (
    <div className="space-y-2">
      <div>
        ...
        <p className="text-slate-500 text-sm">Manajemen data mitra asuransi.</p>
      </div>
...
const UpsertData = ({
  ...
  const [data, setData] = useState<IInsurance>(() => ({
    ...defaultData,
    ...record,
  }));

  const set = (patch: Partial<IInsurance>) =>
    setData((p) => ({ ...p, ...patch }));
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const payload: IInsurance = { ...data, file: serializeFiles(files) };
    await api
      .request({
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            >
              {record ? "Edit Data Asuransi" : "Tambah Asuransi Baru"}
            </p>
            {record && (
        ...
        >
          ...
          <div>
            <label style={FIELD_LABEL_STYLE}>
              Nama Asuransi <span style={{ color: "#ef4444" }}>*</span>
            </label>
            <InputUtil
              label=""
              type="text"
...
const DeleteData = ({
  ...
  return (
    ...
    >
      <div className="p-4">
        <p>
          Konfirmasi hapus data mtira asuransi <strong>{record.name}</strong>?
        </p>
