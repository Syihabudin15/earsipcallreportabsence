...
export default function DataMitra() {
  ...
  const getData = async () => {
    ...
    await api
      .request({
        url: `${import.meta.env.VITE_API_URL}/pay_office?${params}`,
        method: "GET",
      })
      .then((res) =>
        setPageprops((prev) => ({
    ...
    <div className="space-y-2">
      <div>
        ...
        <p className="text-slate-500 text-sm">
          Manajemen data mitra kantor bayar.
        </p>
      </div>
...
const UpsertData = ({
  ...
  const [data, setData] = useState<IPayOffice>(() => ({
    ...defaultData,
    ...record,
  }));

  const set = (patch: Partial<IPayOffice>) =>
    setData((p) => ({ ...p, ...patch }));
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const payload: IPayOffice = { ...data, file: serializeFiles(files) };
    await api
      .request({
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            >
              {record ? "Edit Data Kantor Bayar" : "Tambah Kantor Bayar Baru"}
            </p>
            {record && (
        ...
        >
          ...
          <div>
            <label style={FIELD_LABEL_STYLE}>
              Nama Kantor Bayar <span style={{ color: "#ef4444" }}>*</span>
            </label>
            <InputUtil
              label=""
              type="text"
...
const DeleteData = ({
  ...
  return (
    ...
    >
      <div className="p-4">
        <p>
          Konfirmasi hapus data mitra kantor bayar{" "}
