...
export default function DataVisitStatus() {
  const [loading, setLoading] = useState(false);
  const [pageprops, setPageprops] = useState<IPageProps<IVisitStatus>>({
    page: 1,
    limit: 50,
    data: [],
    total: 0,
  ...
  const [action, setAction] = useState<IActionPage<IVisitStatus>>({
    upsert: false,
    delete: false,
    process: false,
    record: undefined,
  ...
  const getData = async () => {
    ...
    await api
      .request({
        url: `${import.meta.env.VITE_API_URL}/visit_status?${params.toString()}`,
        method: "GET",
      })
      .then((res) =>
        setPageprops((prev) => ({
  ...
  const columns: TableProps<IVisitStatus>["columns"] = [
    {
      title: "ID",
      key: "id",
      dataIndex: "id",
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Manajemen hasil/status kunjungan.
          </p>
        </div>
      </div>
