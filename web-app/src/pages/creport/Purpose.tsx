...
export default function DataVisitPurpose() {
  const [loading, setLoading] = useState(false);
  const [pageprops, setPageprops] = useState<IPageProps<IVisitPurpose>>({
    page: 1,
    limit: 50,
    data: [],
    total: 0,
  ...
  const [action, setAction] = useState<IActionPage<IVisitPurpose>>({
    upsert: false,
    delete: false,
    process: false,
    record: undefined,
  ...
  const getData = async () => {
    ...
    await api
      .request({
        url: `${import.meta.env.VITE_API_URL}/visit_purpose?${params.toString()}`,
        method: "GET",
      })
      .then((res) =>
        setPageprops((prev) => ({
  ...
  const columns: TableProps<IVisitPurpose>["columns"] = [
    {
      title: "ID",
      key: "id",
      dataIndex: "id",
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Manajemen tujuan dilakukannya kunjungan.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-2">
        <div className="bg-white  flex flex-wrap items-center gap-4 mb-2">
          ...
          <div className="flex-1 flex items-center justify-end gap-2">
            <Input.Search
              type="text"
              placeholder="Cari Tujuan Kunjungan/ID..."
...
const UpsertData = ({
  ...
  open: boolean;
  setOpen: Function;
  record?: IVisitPurpose;
  getData: Function;
  hook: HookAPI;
}) => {
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<IVisitPurpose>(record || defaultData);
  ...
  const handleSubmit = async () => {
    ...
    await api
      .request({
        url: import.meta.env.VITE_API_URL + "/visit_purpose?id=" + record?.id,
...
const defaultData: IVisitPurpose = {
  id: "",
  name: "",
  description: "",
  status: true,
