...
const generate = (record: any, selectedMonth: string | null) => {
  ...
  const groupedNplPayment = nplPaymentData.reduce((acc: any, item: any) => {
  ...
  let install7 = 0;
  let pay7 = 0;
  Object.entries(groupedNplPayment).map(([_, items]) => {
    (items as any).forEach((item: any) => {
      install7 += item.tagihan;
      pay7 += item.pembayaran;
      debt7 += 1;
    });
  });
...
<td class="border-excel-light border-total-top border-total-bottom text-center">
        ...
        <table class="text-left">
          <thead>
            <tr class="bg-primary-dark text-white">
              ...
              <th class="border-excel-light text-right">Angsuran Wajib</th>
              <th class="border-excel-light text-center w-24">NPL Gross</th>
              <th class="border-excel-light text-center w-24">NPL / Segment</th>
            </tr>
          </thead>
          <tbody>
            ${segmentKolData
              .map((d, idx) => {
                ...
                const nplseg = isSubtotal ? (d.nplInstansi || 0) / 100 : null;
                ...
                <tr class="${rowClass}">
                  ...
                  <td class="border-excel-light text-center">${nplseg === null ? "" : formatPct(nplseg)}</td>
                </tr>
              `;
              })
              .join("")}
            <tr class="bg-accent-total font-bold text-black">
              ...
              <td colspan="2" class="border-excel-light border-total-top border-total-bottom text-center">${formatPct(globalNplGross2)}</td>
            </tr>
          </tbody>
        </table>
        ${renderSignatures()}
        ...
        <table class="text-left">
          <thead>
            <tr class="bg-primary-dark text-white">
              ...
              <th class="border-excel-light text-right">Angsuran Wajib</th>
              <th class="border-excel-light text-center w-24">NPL Gross</th>
              <th class="border-excel-light text-center w-24">NPL / AO</th>
            </tr>
          </thead>
          <tbody>
            ${aoKolData
              .map((d, idx) => {
                ...
                const rowClass = `${isZebra} ${isSubtotal ? "subtotal-row" : ""}`;
                const nplVal = isSubtotal ? (d.nplGross || 0) / 100 : null;
                const nplAo = isSubtotal ? (d.nplInstansi || 0) / 100 : null;
                const nplFontColor =
                ...
                <tr class="${rowClass}">
                  ...
                  <td class="border-excel-light text-center" style="${nplFontColor}">${nplAo === null ? "" : formatPct(nplAo)}</td>
                </tr>
              `;
              })
              .join("")}
            <tr class="bg-accent-total font-bold text-black">
              ...
              <td colspan="2" class="border-excel-light border-total-top border-total-bottom text-center">${formatPct(globalNplGrossAo)}</td>
            </tr>
          </tbody>
        </table>
        ${renderSignatures()}
  ...
  <div class="text-xs font-bold mb-2" style="color: #1F4E78;">VII. ANALISIS REALISASI PEMBAYARAN TUNGGAKAN NPL</div>
    <table class="text-left w-full">
      ...
      <tbody>
        ${Object.entries(groupedNplPayment)
          .map(([tanggal, items], groupIndex) => {
            const bg = groupIndex % 2 === 0 ? "#FFFFFF" : "#EAF4E2";
        ...
        <tr style="
          background:${bg};
          ${idx === (items as any[]).length - 1 ? "border-bottom:3px solid #1F4E78;" : ""}
        ">
          ${
            idx === 0
              ? `
            <td rowspan="${(items as any[]).length}"
                class="border-excel-light text-center font-bold"
          ...
          <td class="border-excel-light text-center">${d.noLoan}</td>
          ...
          <td class="border-excel-light text-center"
              style="
                font-weight:bold;
                color:${d.status === "BAYAR" ? "#2E7D32" : d.status === "PARTIAL" ? "#1565C0" : "#C62828"};
              ">
            ${d.status}
          </td>
      ...
      `,
        ...
        <tr class="bg-accent-total font-bold text-black">
          ...
          <td class="border-excel-light border-total-top border-total-bottom text-right">${formatIDR(pay7)}</td>
          <td class="border-excel-light border-total-top border-total-bottom text-center"></td>
        </tr>
      </tbody>
    </table>
